...
async def main():
    ...
    async def encryptForCluster(clusterName: str) -> None:
        ...
        for secret in secretData:
            ...
            s1 = s1 + f"\n\n# cmd: echo -n '{secretVal}' | {ks_command} \n# kubesealed secret: value='{secretVal}'\n{encryptedString}"
            s2 = s2 + f"\n{encryptedString}"
        ...
        manifest = manifest_template.format(SCOPE=scope.value, SECRETNAME=secretName.value, NAMESPACE=ns, ENCRYPTEDSECRETS=(allSecrets.rstrip()))
    ...
    async def encryptSecret(clusterName: str, ns: str, secret: dict) -> None:
        ...
        ks_command = f"kubeseal --cert {clusterConfig['clusters'][clusterName]['url']} --scope '{scope.value}' --namespace '{ns}' {ks_param_name} --raw --from-file=/dev/stdin"
    ...
    with ui.card().style(f'width: {def_card_width}'):
        ...
        with ui.row().classes('items-center'):
            ...
            namespace = ui.input(label='Namespace of the Kubernetes Secret', 
                        ...
                        validation={'Input too long: > 64 chars!': lambda value: len(value) < 64, 'Required': lambda value: len(value) > 0})
